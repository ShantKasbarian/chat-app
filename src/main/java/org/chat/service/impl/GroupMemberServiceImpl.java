...
public class GroupMemberServiceImpl implements GroupMemberService {
  ...
  public GroupMember joinGroup(UUID groupId, UserPrincipal userPrincipal) {
    ...
    GroupMember groupMember =
        new GroupMember(
            UUID.randomUUID(), groupId, userId, userPrincipal.username(), GroupMember.Role.PENDING);
  ...
  public GroupMember acceptJoinRequest(UUID userId, UUID groupMemberId) {
    ...
    GroupMember admin =
        groupMemberRepository
            .findByGroupIdUserId(groupMember.getGroupId(), userId)
            .orElseThrow(() -> new ResourceNotFoundException(NOT_MEMBER_OF_GROUP_MESSAGE));
  ...
  public void rejectJoinRequest(UUID userId, UUID groupMemberId) {
    ...
    if (!admin.getRole().equals(GroupMember.Role.ADMIN)
        || !groupMember.getRole().equals(GroupMember.Role.PENDING)) {
      throw new ForbiddenException(REQUEST_NOT_AUTHORIZED);
    }
  ...
  @Override
  public PanacheQuery<GroupMember> findUsersByRole(
