...
class GroupMemberServiceImplTest {
  private static final String ALREADY_MEMBER_OF_GROUP_MESSAGE =
      "you're already a member of this group or have submitted a request to join group";

  private static final String REQUEST_NOT_AUTHORIZED =
  ...
  @InjectMocks private GroupMemberServiceImpl groupUserService;

  @Mock private GroupMemberRepositoryImpl groupUserRepository;
  ...
  @Test
  void joinGroup() {
    when(groupRepository.findByIdOptional(any(UUID.class))).thenReturn(Optional.of(group));
  ...
  void joinGroupShouldThrowResourceNotFoundExceptionWhenGroupNotFound() {
    when(groupRepository.findByIdOptional(any(UUID.class))).thenReturn(Optional.empty());

    Exception exception =
        assertThrows(
            ResourceNotFoundException.class,
            () -> groupUserService.joinGroup(group.getId(), userPrincipal));
  ...
  void acceptJoinRequestShouldThrowResourceNotFoundExceptionWhenAdminNotFound() {
    when(groupUserRepository.findByIdOptional(any(UUID.class))).thenReturn(Optional.empty());

    Exception exception =
        assertThrows(
            ResourceNotFoundException.class,
            () -> groupUserService.acceptJoinRequest(user1.getId(), groupMember2.getId()));
  ...
  void findUsersByRole() {
    when(groupUserRepository.findByRole(
            any(UUID.class), any(GroupMember.Role.class), anyInt(), anyInt()))
        .thenReturn(panacheQuery);

    var response =
        groupUserService.findUsersByRole(
            group.getId(), user1.getId(), GroupMember.Role.MEMBER, 0, 10);

    assertNotNull(response);
    verify(groupUserRepository)
        .findByRole(any(UUID.class), any(GroupMember.Role.class), anyInt(), anyInt());
  }
}
