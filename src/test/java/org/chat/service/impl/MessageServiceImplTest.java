...
class MessageServiceImplTest {
  ...
  @Mock private PanacheQuery<Message> panacheQuery;
  ...
  @Test
  void getMessages() {
    when(messageRepository.findByUserIdTargetUserId(
    ...
    verify(messageRepository)
        .findByUserIdTargetUserId(any(UUID.class), any(UUID.class), anyInt(), anyInt());
  }
  ...
  void getGroupMessagesShouldThrowResourceNotFoundExceptionWhenGroupMemberNotFound() {
    ...
    Exception exception =
        assertThrows(
            ResourceNotFoundException.class,
            () -> messageService.getGroupMessages(group.getId(), user1.getId(), 0, 10));
    assertEquals(NOT_MEMBER_OF_GROUP_MESSAGE, exception.getMessage());
    verify(groupMemberRepository).findByGroupIdUserId(any(UUID.class), any(UUID.class));
  }
